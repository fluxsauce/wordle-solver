...
function loop() {
	...
	prompt.get(["word", "result"], (err, input) => {
		...
		if (!validateWordResult) {
			errors.push(
				`The word must only contain the letters A through Z and be ${length} character(s) long`,
			);
		}
		...
		if (!validateResultResult) {
			errors.push(
				`The result must only contain Y, N or ? and be ${length} character(s) long`,
			);
		}
		...
		exact.forEach((letterPos) => {
			exactPattern =
				exactPattern.substring(0, letterPos.position) +
				letterPos.letter +
				exactPattern.substring(letterPos.position + 1);
		});

		// Negative match.
		const nowhere = not.filter(
			(letter) =>
				!close.some((e) => e.letter === letter) &&
